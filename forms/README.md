...
- **Person Context**: `associated_persons` (everyone, in `sort_order`) and `associated_persons_count`, `household_members` (only those with `lives_in_household`, matching a form's Person 1..8 slots) and `household_members_count`, and `partner` (spouse/partner as a single object, regardless of whether they live in the household). Also `today`, `age`, `is_adult` and `has_reached_retirement_age`. Every person object carries the `associated_persons` columns plus `marital_status_de`/`nationality_de`.
- **Money Grids**: the repeating income/expense/asset tables live in their own rows (`income_entries`, `expense_entries`, `asset_entries`, `benefit_claim_entries`) rather than as one column per form line, and reach a mapping as maps keyed by the enum value naming the form row: `income`, `income_office`, `income_reference`, `expenses`, `expense_notes`, `assets`, `asset_descriptions`, plus `has_any_income`. A key whose name contains a space needs bracket syntax — `{{ expenses['Health Insurance'] }}`. `income_list` is the same income rows ordered rather than keyed, for a form that prints a numbered "n. Art der Einnahme" list; each row carries `income_type_de`, the German wording to print. `pending_benefit_claims` and `expected_one_time_payments` are ordered lists for the two numbered benefit grids. All of these exist both at the top level (the applicant's own) and on every person object (`{{ partner.income.Pension }}`). The one figure a mapping cannot compute is a grid *total* — pyjexl has no reduce — so `monthly_expenses_total` is stored on `users` and `associated_persons` and kept current by the `trg_expense_entries_refresh_total` trigger. Read it, never write it.
